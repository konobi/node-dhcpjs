...
function options_handler (obj, msg, opts) {
  var self = this;

  opts = opts ? opts : {};

  if('direction' in opts && opts.direction == 'decode') {
    return function() {};
  //} else if (opts.direction == 'encode') {
  //} else {
  }
...
module.exports = options_handler;
var builtin_options = [ 1, 2, 3, 4, 5, 6, 12, 15, 50, 53, 52 ]; 
options_handler.builtin_options = builtin_options;


function options_handler_decode () {

  //var order = 
}

function options_handler_encode () {}

options_handler.prototype = {
  // subnetMask - RFC1497
  subNetMask: {
    optionNumber: 1,
  ...
  nameServers: {
    ...
    encode: function (obj, val) {
      ...
      obj.nameServers = new Buffer(bytes);
    },
    decode: function (obj, buf) {
      if((buf.length % 4) == 0) {
        var len = buf.length;
        ...
        var offset = 0;
        while (len > 0) {
          obj.nameServers.push(readIpRaw(buf, offset));
          offset += 4;
          len -= 4;
        }
      } else {
        obj.namServers = undefined;
      }
    }
  },
